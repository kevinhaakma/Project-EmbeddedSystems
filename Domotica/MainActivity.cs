...
namespace Domotica
{
    ...
    public class MainActivity : Activity
    {
        ...
        // Controls on GUI
        Button buttonConnect, device1, device2, device3;
        Button buttonChangePinState;
        TextView textViewServerConnect, textViewTimerStateValue, Status1, Status2, Status3, Sensor1, Sensor2,test;
        public TextView textViewChangePinStateValue, textViewSensorValue, textViewDebugValue;
        EditText editTextIPAddress, editTextIPPort, editInterval;
        ...
        protected override void OnCreate(Bundle bundle)
        {
            ...
            device2 = (Button)FindViewById(Resource.Id.device2);
            device3 = (Button)FindViewById(Resource.Id.device3);

            Status1 = (TextView)FindViewById(Resource.Id.Status1);
            Status2 = (TextView)FindViewById(Resource.Id.Status2);
            Status3 = (TextView)FindViewById(Resource.Id.Status3);

            Sensor1 = (TextView)FindViewById(Resource.Id.Sensor1);
            Sensor2 = (TextView)FindViewById(Resource.Id.Sensor2);

            test = (TextView)FindViewById(Resource.Id.test);

            editInterval = (EditText)FindViewById(Resource.Id.editInterval);

            UpdateConnectionState(4, "Disconnected");

            // Init commandlist, scheduled by socket timer
            commandList.Add(new Tuple<string, TextView>("x", Status1));
            commandList.Add(new Tuple<string, TextView>("y", Status2));
            commandList.Add(new Tuple<string, TextView>("z", Status3));
            commandList.Add(new Tuple<string, TextView>("a", Sensor1));
            commandList.Add(new Tuple<string, TextView>("b", Sensor2));
            ...
            editInterval.TextChanged += (sender, e) =>
            {
                if (editInterval.Text != "")
                    if (Convert.ToInt32(editInterval.Text) >= 1 && Convert.ToInt32(editInterval.Text) <= 10)
            ...
            timerSockets.Elapsed += (obj, args) =>
            {
                RunOnUiThread(() =>
                {
                    if (socket != null) // only if socket exists
                    {
                      ...
                      UpdateGUI(executeCommand(commandList[3].Item1), commandList[3].Item2);  //e.g. UpdateGUI(executeCommand("s"), textViewChangePinStateValue);
                      UpdateGUI(executeCommand(commandList[4].Item1), commandList[4].Item2);  //e.g. UpdateGUI(executeCommand("s"), textViewChangePinStateValue);
                      test.Text = timerSockets.Interval.ToString();
                    }
            ...
            device2.Click += (sender, e) =>
            {
                RunOnUiThread(() =>
                {
                    ...
                    {
                        // Send a command to the Arduino server on every tick (loop though list)
                        UpdateGUI(executeCommand(commandList[1].Item1), commandList[1].Item2);  //e.g. UpdateGUI(executeCommand("x"), textViewChangePinStateValue);
                    }
                });
            };

            device3.Click += (sender, e) =>
            {
                RunOnUiThread(() =>
                {
                    if (socket != null) // only if socket exists
                    {
                        // Send a command to the Arduino server on every tick (loop though list)
                        UpdateGUI(executeCommand(commandList[2].Item1), commandList[2].Item2);  //e.g. UpdateGUI(executeCommand("x"), textViewChangePinStateValue);
                    }
                });
            };
